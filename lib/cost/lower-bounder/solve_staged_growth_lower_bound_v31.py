...
def _highspy_info_snapshot(h: Any) -> dict[str, Any]:
    ...
    for name in (
        'simplex_iteration_count', 'primal_solution_status', 'dual_solution_status',
...
def _highspy_farkas_certificate(
    *,
    ...
    v31 checks every important highspy API status rather than collapsing API
...
def _highspy_direct_feasibility_attempt(
    *,
    ...
    """Solve one direct-feasibility attempt with highspy itself.

    If the same Highs instance reports infeasible, request its dual ray
    immediately, eliminating the second-solve/version mismatch that made v30
    certification hard to interpret.
    """
    try:
        import highspy  # type: ignore
    except Exception as exc:
    ...
    try:
        ...
        if status_lower == 'infeasible':
            ...
            if not request_certificate:
                return meta, None, None
            if not hasattr(h, 'getDualRay'):
                meta['infeasibility_certificate'] = {
                    'available': False, 'verified': False,
                    'reason': 'installed highspy lacks getDualRay()',
                }
                return meta, None, None
            cert_meta: dict[str, Any] = {
                'available': True,
                'verified': False,
                'same_solver_instance': True,
                'highspy_version': meta.get('highspy_version'),
                'time_limit_seconds': (
                    None if certificate_time_limit is None or float(certificate_time_limit) == 0.0
                    else float(certificate_time_limit)
                ),
            }
            # getDualRay() can trigger an additional simplex solve if the initial
            # infeasible solve did not retain a ray. Give that operation the
            # certificate-specific budget, rather than the shorter screening
            # solve budget. Zero means effectively unbounded.
            try:
                ray_time_limit = (
                    1e30 if certificate_time_limit is None or float(certificate_time_limit) == 0.0
                    else float(certificate_time_limit)
                )
                cert_meta['time_limit_option_status'] = str(h.setOptionValue('time_limit', ray_time_limit))
            except Exception as exc:
                cert_meta['time_limit_option_status'] = f'EXCEPTION {type(exc).__name__}: {exc}'
            try:
                ray_start = time.perf_counter()
                raw_ray = h.getDualRay()
                cert_meta['dual_ray_elapsed_seconds'] = float(time.perf_counter() - ray_start)
                cert_meta['get_dual_ray_call_succeeded'] = True
            except Exception as exc:
                cert_meta['get_dual_ray_call_succeeded'] = False
                cert_meta['reason'] = f'highspy getDualRay raised {type(exc).__name__}: {exc}'
                meta['infeasibility_certificate'] = cert_meta
                return meta, None, None
            has_ray, ray_perm, ray_return_shape = _parse_highspy_dual_ray_return(raw_ray)
            cert_meta['get_dual_ray_return'] = ray_return_shape
            cert_meta['has_dual_ray'] = bool(has_ray)
            if not has_ray or ray_perm is None:
                cert_meta['reason'] = 'same-instance highspy solve was infeasible but returned no usable dual ray'
                meta['infeasibility_certificate'] = cert_meta
                return meta, None, None
            ray_perm = np.asarray(ray_perm, dtype=float).reshape(-1)
            if ray_perm.size != m:
                cert_meta['reason'] = f'dual ray length {ray_perm.size} != permuted row count {m}'
                meta['infeasibility_certificate'] = cert_meta
                return meta, None, None
            ...
            cert_meta['verification'] = verification
            cert_meta['verified'] = bool(verification.get('verified', False))
            if not cert_meta['verified']:
                cert_meta['reason'] = 'same-instance dual ray failed independent original-LP Farkas verification'
            meta['infeasibility_certificate'] = cert_meta
            return meta, None, normalized_lambda_original
...
def solve_moment_growth_lp(
    ...
    v31 keeps the fixed-copy technology timing and uses direct zero-objective
    ...
    # Optional explicit Phase-I diagnostics.  v31 defaults to solving the
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v31-pre-solve',
            ...
            if infeasibility_certificate:
                if same_instance_certificate is not None:
                    print(
                        ...
                        f'{_format_elapsed(try_elapsed)}; checking the dual ray from the SAME highspy instance.',
                        flush=True,
                    )
    ...
    output: dict[str, Any] = {
        ...
        'infeasibility_certificate': {
            ...
            'note': (
                'In direct mode, v31 uses highspy as the primary backend when available, so an infeasible '
                'status and dual ray come from the same HiGHS instance/version. If SciPy is explicitly '
                'selected or highspy is unavailable, v31 falls back to a checked highspy re-solve. In '
            ...
            (
                {
                    ...
                    'note': (
                        'v31 direct mode: solve the original feasibility LP directly with zero objective. '
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "tests whether --goal-amount is reachable; v31 defaults to a direct zero-objective "
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "Backend for direct feasibility attempts (default auto). In v31, auto uses "
            "highspy when installed so an infeasible result and its dual ray come from "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable the v31 highspy Farkas-certificate check after a direct HiGHS "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "applies to getDualRay() if it needs an extra simplex solve; with the SciPy "
            "fallback it applies to the highspy certificate re-solve. Set to 0 for no "
    ...
    parser.add_argument(
        ...
        default="direct",
        help=(
            "Feasibility strategy. direct (default in v31) solves the original LP with zero "
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v31",
