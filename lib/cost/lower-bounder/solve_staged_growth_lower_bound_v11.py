...
def solve_moment_growth_lp(
    ...
    v11 eliminates big-M, explicitly equilibrates the final LP, and in feasibility mode minimizes a positive normalized action-flow objective instead of using the identically-zero objective. This leaves the feasible set unchanged but strongly discourages gratuitous huge circulation flows. The goal recipe is split into a required copy
    ...
    if flow_weight_meta is not None:
        print(
            ...
            f"{flow_weight_meta['weight_median']:.3g}",
            flush=True,
        )
    ...
    # Independently re-evaluate the returned point against the ORIGINAL, unscaled
    # inequalities and bounds.  row_scale * raw_residual is dimensionless because
    # each transformed row was normalized before the solve.
    raw_residual = np.asarray(A_ub @ y - b_ub, dtype=float).reshape(-1)
    ...
    validation_tol = 1e-6
    validation_passed = bool(max_norm_violation <= validation_tol and max_bound_violation <= validation_tol)
    output['solution_validation'] = {
    ...
    if not validation_passed:
        output['solver_reported_status'] = output['status']
        output['status'] = 'numerical_difficulty'
        output['solver_message'] = (
            str(output['solver_message']) +
            f'; returned point failed original-LP residual validation (max normalized row violation {max_norm_violation:.3g}, max bound violation {max_bound_violation:.3g})'
        )
        return output
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v11 and ignored. The required-goal-recipe split removes "
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v11",
