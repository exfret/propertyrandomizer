...
def solve_moment_growth_lp(
    ...
    if solve_mode == "feasibility":
        # Ask only whether the optimistic relaxation can reach goal_amount.
        # If not, that is already the lower-bound certificate we need and is
        # often materially cheaper than optimizing the exact relaxed maximum.
        goal_row = np.zeros(total_vars, dtype=float)
        goal_row[:n_actions] = -goal_coeff
        add_group(
            "goal_attainability",
            csr_matrix(goal_row.reshape(1, -1)),
            np.array([x0[goal_index] - goal_amount], dtype=float),
    ...
    print(
        ...
        f"variables={total_vars:,}, inequalities={A_ub.shape[0]:,}, "
        f"A_ub nnz={A_ub.nnz:,}",
        flush=True,
    )
    ...
    print(
        f"HiGHS finished after {_format_elapsed(solve_elapsed)}: {result.message}",
        flush=True,
    )
    ...
    output: dict[str, Any] = {
        ...
        "constraint_matrix_nnz": int(A_ub.nnz),
    ...
    if result.status != 0:
        if result.status == 3 and solve_mode == "maximize":
...
def main() -> None:
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v6",
