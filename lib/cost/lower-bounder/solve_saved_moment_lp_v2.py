        ...
        if solve_meta.get("status") == "infeasible":
            ...
            for cert_backend in certificate_backends:
                if cert_backend == "highspy":
                    ...
                    # SciPy's bundled HiGHS-IPM+crossover has been a useful certificate
                    # polisher on this model, but tiny intermediate buffers can take a
                    # very long time. Try the unbuffered and strongest requested buffer.
                    cert_tol = float(args.feasibility_tolerance)
                    cert_sign_buffers = [0.0, max(sign_buffers)] if max(sign_buffers) > 0.0 else [0.0]
