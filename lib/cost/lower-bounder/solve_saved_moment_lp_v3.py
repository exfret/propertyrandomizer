...
SCHEMA = "propertyrandomizer-standalone-moment-lp-solver-v2"
...
def _configure_highspy(
    ...
    *,
    ...
    opts: list[tuple[str, Any, bool]] = [
        ...
        ("presolve", "on", True),
        ("solver", str(solver_name), True),
        ("run_crossover", "on" if run_crossover else "off", False),
        ("parallel", "off", False),
        ("threads", 1, False),
...
def _highspy_solve_rows(
    *,
    ...
    feasibility_tolerance: float,
    run_crossover: bool = False,
    return_finite_candidate_on_warning: bool = False,
    solver_name: str = "ipm",
    ...
    try:
        ...
        if not _highspy_status_ok(rs):
            if return_finite_candidate_on_warning:
                try:
                    sol = h.getSolution()
                    x = np.asarray(sol.col_value, dtype=float).reshape(-1)
                    if x.size == n and np.all(np.isfinite(x)):
                        meta.update(
                            status="candidate",
                            reason=(
                                f"run returned {rs}; model status={model_status}; "
                                "returning the finite IPM iterate for exact downstream verification"
                            ),
                        )
                        return meta, x
                except Exception as exc:
                    meta["warning_candidate_error"] = f"{type(exc).__name__}: {exc}"
...
def _solve_certificate_lp(
    *,
    ...
    solver_log: bool,
    feasibility_tolerance: float,
    search_method: str = "ipm",
) -> tuple[dict[str, Any], np.ndarray | None]:
    if search_method not in {"ipm", "dual-simplex"}:
        raise ValueError(f"unknown certificate search method {search_method!r}")
    if backend == "scipy":
        options: dict[str, Any] = {
            "disp": bool(solver_log),
            "presolve": True,
        ...
        scipy_method = "highs-ipm" if search_method == "ipm" else "highs-ds"
        if search_method == "ipm":
            options["ipm_optimality_tolerance"] = float(feasibility_tolerance)
        if time_limit is not None and time_limit > 0.0:
            options["time_limit"] = float(time_limit)
        start = time.perf_counter()
        res = linprog(
    ...
    meta, x = _highspy_solve_rows(
        ...
        solver_log=solver_log,
        feasibility_tolerance=feasibility_tolerance,
        run_crossover=(search_method == "ipm"),
        return_finite_candidate_on_warning=True,
        solver_name=("ipm" if search_method == "ipm" else "simplex"),
    )
    meta["search_method"] = search_method
    if x is not None:
...
def _find_verified_certificate(
    *,
    ...
    ordering_attempts: int = 6,
    search_methods: Sequence[str] = ("ipm", "dual-simplex"),
) -> tuple[dict[str, Any], np.ndarray | None]:
    """Search for a certificate candidate and exact-verify every candidate.

    The float certificate LP is only a candidate generator.  Different HiGHS
    versions can return different optimal bases, and exact verification can
    distinguish them even when their floating objectives are nearly identical.
    To avoid depending on one lucky basis, v3 tries both IPM+crossover and
    direct dual simplex, then deterministic row permutations of the same
    certificate problem.  Row permutation changes no mathematics: the
    multiplier is unpermuted before exact verification.
    """
    rounds: list[dict[str, Any]] = []
    m = A_scaled.shape[0]
    max_buffer = max((float(x) for x in sign_buffers), default=0.0)
    methods = [str(x) for x in search_methods]
    for meth in methods:
        if meth not in {"ipm", "dual-simplex"}:
            raise ValueError(f"unknown certificate search method {meth!r}")
    ordering_attempts = max(1, int(ordering_attempts))

    # Deliberately compact schedule.  First try the identity ordering with the
    # unbuffered problem and every requested sign buffer under IPM.  Then try
    # the strongest buffer with direct dual simplex.  If exact verification
    # still rejects the returned basis, change only the row/multiplier ordering
    # and repeat the strongest-buffer searches.  This is proof-safe because no
    # floating status is accepted without exact Fraction verification.
    schedule: list[tuple[int, str, float]] = []
    first_method = "ipm" if "ipm" in methods else methods[0]
    # The strongest small sign buffer has been much more useful than the
    # intermediate values, which can consume tens of seconds while staying on
    # the same fragile face. Try it first, then a different algorithm/basis.
    schedule.append((0, first_method, max_buffer))
    if "dual-simplex" in methods:
        schedule.append((0, "dual-simplex", max_buffer))
    if 0.0 in [float(x) for x in sign_buffers]:
        schedule.append((0, first_method, 0.0))
    for oidx in range(1, ordering_attempts):
        # Alternate equivalent multiplier orderings. Direct dual simplex is
        # preferred for these retries because it lands on a basic candidate
        # without first following the same IPM central path.
        if "dual-simplex" in methods:
            schedule.append((oidx, "dual-simplex", max_buffer))
        if "ipm" in methods:
            schedule.append((oidx, "ipm", max_buffer))

    permutations: dict[int, np.ndarray] = {0: np.arange(m, dtype=int)}
    for oidx in range(1, ordering_attempts):
        rng = np.random.default_rng(np.random.SeedSequence([int(seed), int(oidx), 0xC3A7]))
        permutations[oidx] = np.asarray(rng.permutation(m), dtype=int)

    for attempt_index, (oidx, search_method, sign_buffer) in enumerate(schedule, start=1):
        row_perm = permutations[oidx]
        A_work = A_scaled[row_perm, :].tocsr()
        b_work = np.asarray(b_scaled, dtype=float)[row_perm]
        ...
        rec: dict[str, Any] = {
            "round": attempt_index,
            "ordering_attempt": int(oidx + 1),
            "ordering_seed": int(seed),
            "search_method": search_method,
            "structure": structure,
            "solve": solve_meta,
        }
        rounds.append(rec)
        if x is None or solve_meta.get("status") not in {"optimal", "candidate"}:
            continue
        ...
        lam_perm = np.asarray(x[:m], dtype=float).copy()
        min_lam = float(np.min(lam_perm)) if lam_perm.size else 0.0
        # A solver may put a nonbasic lambda microscopically below its zero
        # bound.  Clipping changes the candidate, so the changed vector is
        # always rechecked from scratch in exact arithmetic.  Do not allow the
        # broad 10*tolerance clipping used by v2; it could materially alter the
        # aggregate on another HiGHS build.
        clip_limit = 1.05 * float(feasibility_tolerance)
        ...
        if min_lam < -clip_limit:
            ...
            continue
        lam_perm[lam_perm < 0.0] = 0.0
        lam_scaled = np.zeros(m, dtype=float)
        lam_scaled[row_perm] = lam_perm
        ...
        if verification.get("verified"):
            return ({
                "available": True,
                "verified": True,
                "method": "multi_basis_box_aware_farkas_plus_exact_fraction_verification",
                "rounds": rounds,
                "successful_round": attempt_index,
                "successful_ordering_attempt": int(oidx + 1),
                "successful_search_method": search_method,
                "verification": verification,
            }, normalized)
        # Make failures visible in the console: v2 hid the one datum needed to
        # understand why an apparently Optimal certificate candidate was not a
        # proof.
        if verification.get("reason") == "aggregate direction is unbounded over a variable box":
            print(
                "    exact cert reject: "
                f"ordering={oidx + 1}, method={search_method}, buffer={sign_buffer:g}, "
                f"column={verification.get('unbounded_column')}, "
                f"aggregate_coeff={verification.get('aggregate_coefficient_float')}",
                flush=True,
            )
        else:
            print(
    ...
    reason = "no candidate basis/order produced an exactly verified row multiplier"
    return ({
        "available": True,
        "verified": False,
        "method": "multi_basis_box_aware_farkas_plus_exact_fraction_verification",
...
def main() -> None:
    ...
    parser.add_argument("--certificate-feasibility-tolerance", type=float, default=1e-6, help="Numerical tolerance used only while searching for certificate candidates (default 1e-6). Safety is unaffected because candidates are exact-verified afterward.")
    parser.add_argument("--certificate-backend", choices=["auto", "highspy", "scipy"], default="auto", help="Backend for certificate search. auto tries the direct backend first and then SciPy as a fallback if exact verification fails.")
    parser.add_argument("--certificate-ordering-attempts", type=int, default=6, help="Number of deterministic row/multiplier orderings tried when certificate candidates fail exact verification (default 6).")
    parser.add_argument("--certificate-search-methods", default="ipm,dual-simplex", help="Comma-separated certificate candidate generators: ipm,dual-simplex (default both).")
    ...
    parser.add_argument(
        ...
        default="0,1e-10,1e-9,1e-8",
        help="Comma-separated relative sign buffers tried by the box-aware certificate LP (default 0,1e-10,1e-9,1e-8)",
    )
    ...
    if args.certificate_feasibility_tolerance <= 0 or not math.isfinite(args.certificate_feasibility_tolerance):
        raise ValueError("--certificate-feasibility-tolerance must be finite and >0")
    ...
    if args.certificate_ordering_attempts < 1:
        raise ValueError("--certificate-ordering-attempts must be >=1")
    certificate_search_methods = [x.strip() for x in args.certificate_search_methods.split(",") if x.strip()]
    if not certificate_search_methods or any(x not in {"ipm", "dual-simplex"} for x in certificate_search_methods):
        raise ValueError("--certificate-search-methods must contain only ipm and/or dual-simplex")
    ...
    for round_index in range(family.max_rounds + 1 if family.enabled else 1):
        ...
        if solve_meta.get("status") == "infeasible":
            ...
            if args.certificate_backend == "auto":
                certificate_backends = [backend]
                if backend != "scipy":
                    certificate_backends.append("scipy")
            else:
                certificate_backends = [args.certificate_backend]
            certificate_attempts: list[dict[str, Any]] = []
            certificate = {"available": True, "verified": False, "reason": "no certificate backend attempted"}
            certified_lambda = None
            for cert_backend in certificate_backends:
                if cert_backend == "highspy":
                    cert_tol = float(args.certificate_feasibility_tolerance)
                    cert_sign_buffers = sign_buffers
                else:
                    # Candidate generation may vary materially across the HiGHS version
                    # bundled with SciPy. v3 therefore keeps every requested buffer and
                    # searches several equivalent row orderings/bases before giving up.
                    cert_tol = float(args.feasibility_tolerance)
                    cert_sign_buffers = sign_buffers
                cert_try, cert_lambda_try = _find_verified_certificate(
                    ...
                    solver_log=args.solver_log,
                    feasibility_tolerance=cert_tol,
                    sign_buffers=cert_sign_buffers,
                    ordering_attempts=int(args.certificate_ordering_attempts),
                    search_methods=certificate_search_methods,
                )
                cert_try = dict(cert_try)
                cert_try["search_backend"] = cert_backend
                certificate_attempts.append(dict(cert_try))
                if cert_try.get("verified") and cert_lambda_try is not None:
                    certificate = dict(cert_try)
                    certified_lambda = cert_lambda_try
                    break
                certificate = dict(cert_try)
            certificate["backend_attempts"] = certificate_attempts
