...
def _highspy_status_ok(status: Any) -> bool:
    """Return True when a highspy call status represents HighsStatus.kOk."""
    if status is None:
        return False
    try:
        # pybind enums compare to integers on some releases.
        if int(status) == 0:
            return True
    except Exception:
        pass
    text = str(status).strip().lower().replace(' ', '')
    return text in {'ok', 'kok', 'highsstatus.kok', 'highsstatus::kok'} or text.endswith('.kok')


def _highspy_reset_global_scheduler(highspy_module: Any) -> dict[str, Any]:
    """Reset HiGHS' process-global scheduler before/after serialized solves.

    HiGHS keeps a global scheduler for thread management. Repeated solver
    instances in the same Python process can otherwise inherit scheduler state
    that is surprising when threads/parallel options change. Failure to reset
    is diagnostic only; it never changes certificate validity.
    """
...
def _highspy_build_lp(
    ...
    *,
    ...
    """Build a HighsLp with A x <= b and box bounds, preserving float64 data."""
    A_csr = A.tocsr()
    m, n = A_csr.shape
    inf = float(getattr(highspy_module, 'kHighsInf', 1e30))
    ...
    row_hi = np.asarray(b, dtype=np.double)

    lp = highspy_module.HighsLp()
    lp.num_col_ = int(n)
    lp.num_row_ = int(m)
    lp.col_cost_ = (
        np.zeros(n, dtype=np.double)
        if c is None else np.asarray(c, dtype=np.double)
    )
    ...
    # Pass the matrix column-wise. int32 matches the default HighsInt build and
    # is more portable than relying on pybind to cast int64 sparse indices.
    Ac = A_csr.tocsc()
    try:
        lp.a_matrix_.format_ = highspy_module.MatrixFormat.kColwise
    except Exception:
        pass
...
def _highspy_configure(
    ...
    *,
    ...
    """Set deterministic serialized LP options and record every call status.

    v33 uses IPM by default for the direct feasibility path.  A solver status by
    itself is never used as an infeasibility proof: infeasible IPM reports must
    still pass the independent Farkas-certificate verifier below.
    """
    solver = str(solver).strip().lower()
    if solver not in {'ipm', 'simplex'}:
        raise ValueError(f'Unsupported highspy solver {solver!r}; expected ipm or simplex')
    option_statuses: dict[str, str] = {}
    required_failures: list[str] = []
    options: list[tuple[str, Any, bool]] = [
        ('output_flag', bool(output_flag), False),
        ('presolve', str(presolve), True),
        ('solver', solver, True),
        # Do not hand an IPM optimum to simplex crossover: the whole point of the
        # v33 path is to avoid the unstable basis trajectory on this model.
        ('run_crossover', 'off' if solver == 'ipm' else 'on', False),
        ('parallel', 'off', False),
        ('threads', 1, False),
        ('random_seed', int(attempt_seed % 2147483647), False),
        # The explicitly scaled/lazy-cut LP can contain meaningful coefficients
        # between 1e-12 and HiGHS' default 1e-9 cutoff. Preserve them rather than
        # silently changing the mathematical problem.
        ('small_matrix_value', 1e-12, True),
    ]
    if time_limit_seconds is not None and float(time_limit_seconds) > 0.0:
        options.append(('time_limit', float(time_limit_seconds), False))
    for name, value, required in options:
        try:
            status = h.setOptionValue(name, value)
            option_statuses[name] = str(status)
            if required and not _highspy_status_ok(status):
                required_failures.append(f'{name}={status}')
        except Exception as exc:
            option_statuses[name] = f'EXCEPTION {type(exc).__name__}: {exc}'
            if required:
                required_failures.append(f'{name}=exception')
    return option_statuses, required_failures


def _highspy_info_snapshot(h: Any) -> dict[str, Any]:
    """Best-effort small diagnostic snapshot from HighsInfo."""
    out: dict[str, Any] = {}
    try:
        info = h.getInfo()
    except Exception as exc:
        return {'get_info_error': f'{type(exc).__name__}: {exc}'}
    for name in (
        'simplex_iteration_count', 'ipm_iteration_count', 'crossover_iteration_count',
        'primal_solution_status', 'dual_solution_status',
        'basis_validity', 'num_primal_infeasibilities', 'max_primal_infeasibility',
        'sum_primal_infeasibilities', 'num_dual_infeasibilities',
...
def _verify_farkas_row_ray(
    *,
    ...
    for orientation in (1.0, -1.0):
        ...
        # Do not reject on the longdouble sign screen: coefficients near zero can
        # change sign purely from summation order.  Only the exact Decimal-from-
        # float recomputation below is authoritative.
        ...
        cand = {
            ...
            'longdouble_screen_gap': (
                None if unbounded_column is not None else float(min_lhs_ld - rhs_ld)
            ),
            'longdouble_unbounded_column': (
                int(unbounded_column) if unbounded_column is not None else None
            ),
...
def _highspy_ipm_farkas_certificate(
    *,
    ...
    """Find and independently verify a Farkas certificate with an IPM LP.

    For the scaled/permuted feasibility system A z <= b with box bounds, append
    each finite box bound as an inequality to obtain B z <= d.  Farkas' lemma
    says infeasibility is equivalent to the existence of y >= 0 with

        B^T y = 0,   d^T y < 0.

    Normalize sum(y)=1 and minimize d^T y.  This is another ordinary LP, which
    v33 solves with HiGHS IPM.  The IPM objective/status is *not* itself trusted:
    only the ordinary-row part of y is mapped back through permutation/scaling
    and accepted if _verify_farkas_row_ray proves a strict contradiction against
    the original unscaled float64 LP using high-precision arithmetic.
    """
    try:
    ...
    b = np.asarray(b_scaled_permuted, dtype=float).reshape(-1)
    m, n = A.shape
    if b.size != m or len(bounds_scaled_permuted) != n:
        return ({
            'available': True, 'verified': False,
            'reason': 'certificate LP dimension mismatch',
        }, None)

    # B = [ordinary A rows; finite upper-bound rows; finite lower-bound rows].
    bound_rr: list[int] = []
    bound_cc: list[int] = []
    bound_vv: list[float] = []
    bound_rhs: list[float] = []
    bound_kind: list[dict[str, Any]] = []
    br = 0
    for j, (lo, hi) in enumerate(bounds_scaled_permuted):
        if hi is not None and math.isfinite(float(hi)):
            bound_rr.append(br); bound_cc.append(j); bound_vv.append(1.0)
            bound_rhs.append(float(hi))
            bound_kind.append({'column': int(j), 'kind': 'upper', 'value': float(hi)})
            br += 1
        if lo is not None and math.isfinite(float(lo)):
            bound_rr.append(br); bound_cc.append(j); bound_vv.append(-1.0)
            bound_rhs.append(-float(lo))
            bound_kind.append({'column': int(j), 'kind': 'lower', 'value': float(lo)})
            br += 1
    Bbounds = coo_matrix(
        (bound_vv, (bound_rr, bound_cc)), shape=(br, n), dtype=float
    ).tocsr()
    B = vstack([A, Bbounds], format='csr') if br else A.copy()
    d = np.concatenate([b, np.asarray(bound_rhs, dtype=float)]) if br else b.copy()
    p = B.shape[0]

    # Certificate LP variables are y_0..y_{p-1}.  Equality rows are B^T y=0
    # plus sum(y)=1.
    M = vstack([
        B.T.tocsr(),
        csr_matrix(np.ones((1, p), dtype=float)),
    ], format='csr')
    row_lo = np.concatenate([np.zeros(n, dtype=float), np.ones(1, dtype=float)])
    row_hi = row_lo.copy()
    ...
    lp.num_col_ = int(p)
    lp.num_row_ = int(n + 1)
    lp.col_cost_ = np.asarray(d, dtype=np.double)
    lp.col_lower_ = np.zeros(p, dtype=np.double)
    lp.col_upper_ = np.full(p, inf, dtype=np.double)
    lp.row_lower_ = np.asarray(row_lo, dtype=np.double)
    lp.row_upper_ = np.asarray(row_hi, dtype=np.double)
    Mc = M.tocsc()
    try:
        lp.a_matrix_.format_ = highspy.MatrixFormat.kColwise
    except Exception:
        pass
    lp.a_matrix_.start_ = np.asarray(Mc.indptr, dtype=np.int32)
    lp.a_matrix_.index_ = np.asarray(Mc.indices, dtype=np.int32)
    lp.a_matrix_.value_ = np.asarray(Mc.data, dtype=np.double)

    meta: dict[str, Any] = {
        'available': True,
        'verified': False,
        'method': 'explicit_farkas_lp_ipm',
        'attempt_seed': int(attempt_seed),
        'ordinary_rows': int(m),
        'finite_bound_rows': int(br),
        'certificate_variables': int(p),
        'certificate_equalities': int(n + 1),
        'time_limit_seconds': (
            None if time_limit_seconds is None or float(time_limit_seconds) == 0.0
            else float(time_limit_seconds)
        ),
    }
    meta['global_scheduler_reset_before'] = _highspy_reset_global_scheduler(highspy)
    try:
        h = highspy.Highs()
        try:
        ...
        option_statuses, required_failures = _highspy_configure(
            h,
            attempt_seed=attempt_seed,
            time_limit_seconds=time_limit_seconds,
            output_flag=bool(solver_log),
            presolve='on',
            solver='ipm',
        )
        meta['option_statuses'] = option_statuses
        if required_failures:
            meta['reason'] = 'required highspy option setup failed: ' + ', '.join(required_failures)
            return meta, None
        pass_status = h.passModel(lp)
        meta['pass_model_status'] = str(pass_status)
        if not _highspy_status_ok(pass_status):
            meta['reason'] = f'highspy passModel failed ({pass_status})'
            return meta, None
        start = time.perf_counter()
        run_status = h.run()
        meta['solve_elapsed_seconds'] = float(time.perf_counter() - start)
        meta['run_status'] = str(run_status)
        meta['info'] = _highspy_info_snapshot(h)
        try:
            model_status_text = str(h.modelStatusToString(h.getModelStatus()))
        except Exception as exc:
            model_status_text = f'ERROR {type(exc).__name__}: {exc}'
        meta['model_status'] = model_status_text
        if not _highspy_status_ok(run_status):
            meta['reason'] = f'certificate IPM run failed ({run_status}); model status={model_status_text}'
            return meta, None
        if model_status_text.strip().lower() != 'optimal':
            meta['reason'] = f'certificate IPM did not return Optimal ({model_status_text})'
            return meta, None
        try:
            sol = h.getSolution()
            y = np.asarray(sol.col_value, dtype=float).reshape(-1)
        except Exception as exc:
            meta['reason'] = f'certificate IPM getSolution failed: {type(exc).__name__}: {exc}'
            return meta, None
        if y.size != p or not np.all(np.isfinite(y)):
            meta['reason'] = f'certificate IPM returned invalid multiplier vector length {y.size}; expected {p}'
            return meta, None
        cert_obj = float(np.dot(d, y))
        eq_resid = np.asarray(B.T @ y, dtype=float).reshape(-1)
        meta['candidate_objective_dTy'] = cert_obj
        meta['candidate_sum_y'] = float(np.sum(y))
        meta['candidate_min_y'] = float(np.min(y)) if y.size else 0.0
        meta['candidate_max_abs_BTy'] = float(np.max(np.abs(eq_resid))) if eq_resid.size else 0.0
        if not math.isfinite(cert_obj) or cert_obj >= 0.0:
            meta['reason'] = f'certificate LP optimum is not negative (d^T y={cert_obj:.12g})'
            return meta, None

        lam_perm = np.asarray(y[:m], dtype=float)
        lam_scaled_original_order = np.zeros(m, dtype=float)
        lam_scaled_original_order[np.asarray(row_perm, dtype=int)] = lam_perm
        lam_original = lam_scaled_original_order * np.asarray(row_scale, dtype=float)
        verification, normalized_lambda_original = _verify_farkas_row_ray(
            ...
            row_ray_original=lam_original,
        )
        meta['verification'] = verification
        meta['verified'] = bool(verification.get('verified', False))
        if not meta['verified']:
            meta['reason'] = 'IPM Farkas candidate failed independent original-LP verification'
        return meta, normalized_lambda_original
    except Exception as exc:
        meta['reason'] = f'IPM Farkas certificate path raised {type(exc).__name__}: {exc}'
        return meta, None
    finally:
        try:
            meta['global_scheduler_reset_after'] = _highspy_reset_global_scheduler(highspy)
        except Exception:
            pass
...
def _highspy_farkas_certificate(
    *,
    ...
    v33 checks every important highspy API status rather than collapsing API
    failures into model status ``Not Set``. It also resets HiGHS' global
    scheduler before/after the serialized certificate solve. Any ray is still
    independently verified against the original unscaled LP.
    """
    try:
        import highspy  # type: ignore
    except Exception as exc:
    ...
    meta: dict[str, Any] = {
        ...
        'attempt_seed': int(attempt_seed),
        'highs_random_seed': int(attempt_seed % 2147483647),
    }
    scheduler_before = _highspy_reset_global_scheduler(highspy)
    meta['global_scheduler_reset_before'] = scheduler_before
    h = None
    try:
        h = highspy.Highs()
        try:
        ...
        option_statuses, required_option_failures = _highspy_configure(
            h,
            attempt_seed=attempt_seed,
            time_limit_seconds=time_limit_seconds,
            output_flag=bool(solver_log),
        ...
        meta['option_statuses'] = option_statuses
        if required_option_failures:
            meta['reason'] = 'required highspy option setup failed: ' + ', '.join(required_option_failures)
            return meta, None

        A = A_scaled_permuted.tocsr()
        m, n = A.shape
        lp = _highspy_build_lp(
            highspy,
            A=A,
            b=b_scaled_permuted,
            bounds=bounds_scaled_permuted,
            c=np.zeros(n, dtype=np.double),
        )
        pass_status = h.passModel(lp)
        meta['pass_model_status'] = str(pass_status)
        meta['model_dimensions'] = {'rows': int(m), 'cols': int(n), 'nnz': int(A.nnz)}
        if not _highspy_status_ok(pass_status):
            meta['reason'] = f'highspy passModel failed ({pass_status})'
            return meta, None
        ...
        elapsed = time.perf_counter() - start
        meta['run_status'] = str(run_status)
        meta['solve_elapsed_seconds'] = float(elapsed)
        meta['info'] = _highspy_info_snapshot(h)
        try:
        ...
        if not _highspy_status_ok(run_status):
            meta['reason'] = (
                f'highspy run failed ({run_status}); model status={model_status_text}. '
                'This is a solver/API failure, not evidence of feasibility.'
            )
            return meta, None
        ...
        try:
            ...
            meta['get_dual_ray_call_succeeded'] = True
        except Exception as exc:
            meta['get_dual_ray_call_succeeded'] = False
            meta['reason'] = f'highspy getDualRay raised {type(exc).__name__}: {exc}'
            return meta, None
        ...
        # scaled row is r_i A_i x <= r_i b_i, a ray multiplier lambda_i on that
        # row corresponds to lambda_i*r_i on the original row.
...
def _highspy_direct_feasibility_attempt(
    *,
    A_scaled_permuted: csr_matrix,
    b_scaled_permuted: np.ndarray,
    bounds_scaled_permuted: Sequence[tuple[float | None, float | None]],
    ...
    certificate_time_limit: float | None,
    solver_log: bool,
    request_certificate: bool,
    solver: str = 'ipm',
) -> tuple[dict[str, Any], np.ndarray | None, np.ndarray | None]:
    """Solve one direct-feasibility attempt with highspy.

    v33 defaults to IPM because the moment LP is much more stable under IPM than
    dual simplex.  An IPM infeasibility status is only a *screening* result.  If
    certification is requested, v33 solves the explicit normalized Farkas LP
    with IPM and independently verifies its multiplier against the original
    unscaled LP.  Thus IPM cannot manufacture a false lower-bound proof.
    """
    try:
        import highspy  # type: ignore
    except Exception as exc:
    ...
    if solver not in {'ipm', 'simplex'}:
        return ({
            'available': True, 'status': 'api_error',
            'reason': f'unsupported highspy solver {solver!r}',
        }, None, None)
    ...
    meta: dict[str, Any] = {
        ...
        'attempt_seed': int(attempt_seed),
        'highs_random_seed': int(attempt_seed % 2147483647),
        'certificate_requested': bool(request_certificate),
        'infeasibility_status_is_proof': False,
    }
    meta['global_scheduler_reset_before'] = _highspy_reset_global_scheduler(highspy)
    try:
        h = highspy.Highs()
        ...
        if not _highspy_status_ok(run_status):
            meta['status'] = 'api_error'
            meta['reason'] = f'highspy run failed ({run_status}); model status={model_status_text}'
            return meta, None, None

        status_lower = model_status_text.strip().lower()
        if status_lower == 'optimal':
            try:
                ...
                meta['status'] = 'api_error'
                meta['reason'] = f'highspy optimal solve but getSolution failed: {type(exc).__name__}: {exc}'
                return meta, None, None
            if x.size != n or not np.all(np.isfinite(x)):
                meta['status'] = 'api_error'
                meta['reason'] = f'highspy returned invalid solution vector of length {x.size}; expected {n}'
                return meta, None, None
            meta['status'] = 'optimal'
            return meta, x, None

        if status_lower == 'infeasible':
            meta['status'] = 'infeasible'
            if not request_certificate:
                return meta, None, None
            cert_meta, cert_lambda = _highspy_ipm_farkas_certificate(
                A_scaled_permuted=A,
                b_scaled_permuted=b_scaled_permuted,
                bounds_scaled_permuted=bounds_scaled_permuted,
                row_perm=row_perm,
                row_scale=row_scale,
                A_original=A_original,
                b_original=b_original,
            ...
            cert_meta['triggered_by_same_attempt'] = True
            cert_meta['screening_solver'] = solver
            meta['infeasibility_certificate'] = cert_meta
            return meta, None, cert_lambda

        # Time/iteration limits and numerical statuses are inconclusive.
        meta['status'] = 'nondecisive'
        meta['reason'] = f'highspy returned model status {model_status_text}'
        return meta, None, None
    except Exception as exc:
        meta['status'] = 'api_error'
        meta['reason'] = f'highspy direct path raised {type(exc).__name__}: {exc}'
        return meta, None, None
    finally:
        try:
            meta['global_scheduler_reset_after'] = _highspy_reset_global_scheduler(highspy)
...
def solve_moment_growth_lp(
    ...
    direct_solver_backend: str = "auto",
    relax_energy_flow: bool = True,
    lazy_tail_max_rounds: int = 50,
    keep_generic_u_tech_gates: bool = False,
    ...
    v33 keeps the fixed-copy technology timing and uses direct zero-objective
    ...
    if lazy_tail_max_rounds < 0:
        raise ValueError('lazy_tail_max_rounds must be >= 0')
    ...
    # Energy is deliberately free in v33 by default.  The earlier energy-source
    # carrier coordinates used joule-scale coefficients (1e5--1e7 per operation
    # second) and dominated the matrix dynamic range, while energy infrastructure
    # is not currently part of the lower-bound claim.  Projecting every real
    # schedule onto a model that simply omits energy expenditure is an optimistic
    # relaxation, so it cannot create a false lower bound.  Keep the coordinates
    # for stable indexing/debugging, but zero their flow columns and force the now-
    # useless energy-production actions to zero.
    energy_flow_meta: dict[str, Any] = {
        'relaxed_free': bool(relax_energy_flow),
        'material_keys': [],
        'producer_action_keys': [],
        'zeroed_S_nonzeros': 0,
    }
    energy_producer_action_indices: set[int] = set()
    energy_material_indices: list[int] = []
    if relax_energy_flow:
        energy_material_indices = [
            i for i, key in enumerate(col_keys)
            if key.startswith('energy-source-')
            or key.startswith('__pr_graph_lp_and_output__: energy-source-')
        ]
        energy_flow_meta['material_keys'] = [str(col_keys[i]) for i in energy_material_indices]
        if energy_material_indices:
            S = S.tolil(copy=True)
            for j in energy_material_indices:
                energy_flow_meta['zeroed_S_nonzeros'] += int(S[:, int(j)].getnnz())
                # Any action that positively produced this carrier becomes unnecessary
                # once energy is free. Record it before erasing the carrier column.
                col = S[:, int(j)].tocoo()
                for rr, vv in zip(col.row, col.data):
                    if float(vv) > 0.0:
                        energy_producer_action_indices.add(int(rr))
                S[:, int(j)] = 0.0
            S = S.tocsr(); S.eliminate_zeros()
        energy_flow_meta['producer_action_keys'] = [
            str(row_keys[i]) for i in sorted(energy_producer_action_indices)
        ]
    ...
    if relax_energy_flow:
        infinite_indices.update(int(i) for i in energy_material_indices)
    ...
    nonstorable_carrier_indices = np.asarray([
        i for i, key in enumerate(col_keys)
        if int(i) not in set(energy_material_indices)
        and (
            key.startswith('__pr_graph_lp_and_output__:')
    ...
    # Recipes that already have a positive player-informed amount lower bound get
    # the much better-conditioned L_r moment timing cuts below.  Do not also add
    # the older U_r availability hierarchy for them: those rows can contain huge
    # U_r*delta coefficients and were a major source of within-row dynamic range.
    # Dropping a valid constraint family is an optimistic relaxation, hence safe
    # for a lower-bound proof.
    generic_recipe_constraints = (
        [
            (int(ri), recipe, tech)
            for ri, recipe, tech in recipe_constraints
            if float(action_q0_lower_bounds_by_index.get(int(ri), 0.0)) <= 0.0
        ]
        if keep_generic_u_tech_gates else []
    )
    ...
    recipe_rate_bound_meta: dict[str, Any] = {
        'enabled': bool(generic_recipe_constraints),
        ...
        'all_gated_recipe_count': len(recipe_constraints),
        'generic_u_gates_enabled': bool(keep_generic_u_tech_gates),
        'skipped_due_to_positive_exported_lower_bound': (
            int(sum(1 for ri, _recipe, _tech in recipe_constraints
                    if float(action_q0_lower_bounds_by_index.get(int(ri), 0.0)) > 0.0))
        ),
        'skipped_because_generic_u_hierarchy_disabled': (
            int(sum(1 for ri, _recipe, _tech in recipe_constraints
                    if float(action_q0_lower_bounds_by_index.get(int(ri), 0.0)) <= 0.0))
            if not keep_generic_u_tech_gates else 0
        ),
        'generic_recipe_count': len(generic_recipe_constraints),
    }
    if generic_recipe_constraints:
        if building_placement_rate is None:
            for _ri, recipe, _tech in generic_recipe_constraints:
            ...
            if static_parts:
                ...
                for ri, recipe, _tech in generic_recipe_constraints:
    ...
    # v33 separates this continuous family lazily in feasibility mode instead of
    # preloading a geometric grid down to one Factorio tick.  The tiny-D grid rows
    # had internal coefficient ratios up to ~1e8 even after row scaling.  Given a
    # candidate solution, the violation is a concave quadratic in D, so its exact
    # maximizer can be found analytically and one globally valid row added.
    _tail_min_window = min(float(time_seconds), 1.0 / 60.0)
    ...
    recipe_tail_capacity_meta['mode'] = (
        'lazy_continuous_separation' if solve_mode == 'feasibility' else 'static_geometric_grid'
    )
    recipe_tail_capacity_meta['minimum_window_seconds'] = float(_tail_min_window)
    recipe_tail_capacity_meta['window_seconds'] = [float(x) for x in _tail_windows]
    recipe_tail_capacity_meta['lazy_rounds'] = 0
    recipe_tail_capacity_meta['lazy_rows_added'] = 0
    recipe_tail_capacity_meta['lazy_max_normalized_violation_before_cut'] = 0.0
    ...
    def _tail_row_dict(spec: dict[str, Any], D: float) -> tuple[dict[int, float], float]:
        ri = int(spec['recipe_action_index'])
        csec = float(spec['category_seconds_per_run'])
        row: dict[int, float] = {
            ri: 2.0 * float(D),
            n_actions + ri: -2.0 * float(time_seconds),
        }
        rate_factor = (float(D) * float(D)) / csec
        for aj, coeff in spec['rate_coeff'].items():
            idx = int(aj)
            row[idx] = row.get(idx, 0.0) - rate_factor * float(coeff)
        return row, rate_factor * float(spec['constant_rate'])

    # Maximize the exact continuous-family violation for one candidate.  With
    # R = constant + sum rate_coeff*q0, f(D)=2D*q0-2T*q1-(D^2/c)R is concave.
    def _tail_most_violated(
        ...
        ri = int(spec['recipe_action_index'])
        csec = float(spec['category_seconds_per_run'])
        q0 = max(0.0, float(y_candidate[ri]))
        q1 = max(0.0, float(y_candidate[n_actions + ri]))
        R = float(spec['constant_rate'])
        for aj, coeff in spec['rate_coeff'].items():
            R += float(coeff) * max(0.0, float(y_candidate[int(aj)]))
        R = max(0.0, R)
        candidates = [_tail_min_window, float(time_seconds)]
        if R > 0.0 and q0 > 0.0:
            candidates.append(min(float(time_seconds), max(_tail_min_window, csec * q0 / R)))
        best_D = candidates[0]
        best_v = -math.inf
        best_scale = 1.0
        for D in candidates:
            lhs1 = 2.0 * D * q0
            lhs2 = 2.0 * float(time_seconds) * q1
    ...
    # In maximize mode retain the old static grid for compatibility.  Feasibility
    # mode starts without any tail rows and separates the exact continuous family
    # only when a candidate actually violates it.
    recipe_tail_capacity_row_count = 0
    if recipe_tail_capacity_specs and moment_order >= 1 and solve_mode != 'feasibility':
        for D in _tail_windows:
            rows: list[dict[int, float]] = []
            rhs: list[float] = []
            for spec in recipe_tail_capacity_specs:
                row, brhs = _tail_row_dict(spec, float(D))
                rows.append(row); rhs.append(brhs)
            ...
            add_sparse_rows(
                ...
                formulation=recipe_tail_capacity_meta['formulation'],
            )
    ...
    if relax_energy_flow and energy_producer_action_indices:
        for ai in sorted(energy_producer_action_indices):
            for k in range(blocks):
                bounds[k * n_actions + int(ai)] = (0.0, 0.0)
    ...
    # v33 explicitly asks HiGHS to preserve matrix coefficients down to 1e-12.
    # Never proceed if scaling creates nonzero terms at or below that configured
    # cutoff, because silently deleting them would change the mathematical LP.
    abs_scaled_data = np.abs(A_solve.data)
    tiny_1e12 = int(np.count_nonzero(abs_scaled_data <= 1e-12)) if abs_scaled_data.size else 0
    ...
    scaling_meta = dict(scaling_meta)
    scaling_meta['coefficients_le_1e-12'] = tiny_1e12
    scaling_meta['coefficients_le_1e-9'] = tiny_1e9
    scaling_meta['coefficients_le_1e-8'] = tiny_1e8
    scaling_meta['configured_highs_small_matrix_value'] = 1e-12
    if tiny_1e12:
        raise RuntimeError(
            f'Explicit LP scaling produced {tiny_1e12} nonzero coefficient(s) <= 1e-12. '
            'HiGHS is configured to treat such entries as zero, changing the feasibility problem. '
            'Use a less aggressive scaling rule rather than accepting this solve.'
        )

    # Lazy continuous separation state for the recipe tail-capacity family.
    lazy_tail_cut_keys: set[tuple[int, int]] = set()
    lazy_tail_rounds_total = 0
    lazy_tail_separation_tolerance = 1e-6

    def _append_violated_lazy_tail_cuts(y_candidate: np.ndarray) -> list[dict[str, Any]]:
        """Append one exact most-violated D cut per offending recipe/category pair.

        Returns metadata for newly appended rows.  Recomputing the exact diagonal
        scaling after each batch is cheap compared with a solve and guarantees the
        new rows are treated consistently with the base LP.
        """
        nonlocal A_ub, b_ub, A_solve, b_solve, c_solve, bounds_solve
        nonlocal row_scale, column_scale, scaling_meta, recipe_tail_capacity_row_count
        nonlocal lazy_tail_rounds_total
        if solve_mode != 'feasibility' or moment_order < 1 or not recipe_tail_capacity_specs:
            return []
        if lazy_tail_rounds_total >= int(lazy_tail_max_rounds):
            raise RuntimeError(
                f'Lazy recipe-tail separation exceeded --lazy-tail-max-rounds={lazy_tail_max_rounds}; '
                'refusing to accept a candidate without checking the full tail family.'
            )
        rows: list[dict[int, float]] = []
        rhs: list[float] = []
        meta_rows: list[dict[str, Any]] = []
        max_norm = 0.0
        for si, spec in enumerate(recipe_tail_capacity_specs):
            D, violation, normalized_violation = _tail_most_violated(y_candidate, spec)
            max_norm = max(max_norm, float(normalized_violation))
            if normalized_violation <= lazy_tail_separation_tolerance:
                continue
            # Quantize only for duplicate detection, never for the coefficient used.
            dkey = int(round(float(D) * 1e12))
            key = (int(si), dkey)
            if key in lazy_tail_cut_keys:
                continue
            row, brhs = _tail_row_dict(spec, float(D))
            rows.append(row); rhs.append(brhs)
            meta_rows.append({
                'spec_index': int(si),
                'recipe_action_key': str(spec['recipe_action_key']),
                'recipe_category_key': str(spec['recipe_category_key']),
                'window_seconds': float(D),
                'candidate_original_violation': float(violation),
                'candidate_normalized_violation': float(normalized_violation),
            })
            lazy_tail_cut_keys.add(key)
        recipe_tail_capacity_meta['lazy_max_normalized_violation_before_cut'] = max(
            float(recipe_tail_capacity_meta.get('lazy_max_normalized_violation_before_cut', 0.0)),
            float(max_norm),
        )
        if not rows:
            return []
        ...
        for i, row in enumerate(rows):
            for j, value in row.items():
                if value != 0.0:
                    rr.append(i); cc.append(int(j)); vv.append(float(value))
        mat = coo_matrix((vv, (rr, cc)), shape=(len(rows), total_vars), dtype=float).tocsr()
        start_row = int(A_ub.shape[0])
        A_ub = vstack([A_ub, mat], format='csr')
        b_ub = np.concatenate([b_ub, np.asarray(rhs, dtype=float)])
        constraint_groups.append({
            'name': f'recipe_tail_capacity_lazy_round_{lazy_tail_rounds_total + 1}',
            'row_start': start_row,
            'row_stop_exclusive': start_row + len(rows),
            'lazy': True,
            'formulation': recipe_tail_capacity_meta['formulation'],
            'rows': meta_rows,
        })
        recipe_tail_capacity_row_count += len(rows)
        lazy_tail_rounds_total += 1
        recipe_tail_capacity_meta['lazy_rounds'] = int(lazy_tail_rounds_total)
        recipe_tail_capacity_meta['lazy_rows_added'] = int(recipe_tail_capacity_row_count)
        recipe_tail_capacity_meta['window_seconds'] = [
            float(item['window_seconds'])
            for group in constraint_groups if group.get('lazy')
            for item in group.get('rows', [])
        ]

        A_solve, b_solve, c_solve, bounds_solve, row_scale, column_scale, new_scaling_meta = (
            _scale_lp_rows_and_columns(
                A_ub, b_ub, c, bounds, lp_scaling,
                semantic_column_scale=semantic_full_scale,
            )
        ...
        if semantic_scale_meta is not None:
            new_scaling_meta = dict(new_scaling_meta)
            new_scaling_meta['semantic_action_scaling'] = semantic_scale_meta
        abs_data = np.abs(A_solve.data)
        tiny = int(np.count_nonzero(abs_data <= 1e-12)) if abs_data.size else 0
        new_scaling_meta = dict(new_scaling_meta)
        new_scaling_meta['coefficients_le_1e-12'] = tiny
        new_scaling_meta['configured_highs_small_matrix_value'] = 1e-12
        if tiny:
            raise RuntimeError(
                f'Lazy tail separation produced {tiny} nonzero scaled coefficient(s) <= 1e-12; '
                'refusing to let HiGHS silently delete a valid cut term.'
            )
        scaling_meta = new_scaling_meta
        if save_lp_dir is not None:
            save_npz(save_lp_dir / 'A-ub-with-lazy-cuts.npz', A_ub)
            np.save(save_lp_dir / 'b-ub-with-lazy-cuts.npy', b_ub)
            save_npz(save_lp_dir / 'A-ub-scaled-with-lazy-cuts.npz', A_solve)
            np.save(save_lp_dir / 'b-ub-scaled-with-lazy-cuts.npy', b_solve)
            np.save(save_lp_dir / 'row-scale-with-lazy-cuts.npy', row_scale)
        return meta_rows
    ...
    # Optional explicit Phase-I diagnostics.  v33 defaults to solving the
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v33-pre-solve',
        ...
        # Standalone numerical-solver package.  This is the stable interface
        # between the Factorio/model-building code and solve_saved_moment_lp_v1.py.
        # The base A/b intentionally omit feasibility-mode recipe-tail rows; the
        # continuous tail family is represented by the specs below and separated
        # lazily by the standalone solver.
        solver_package = {
            'schema': 'propertyrandomizer-saved-moment-lp-v1',
            'builder_format': 'propertyrandomizer-moment-growth-lp-v33',
            'base_matrix': 'A-ub.npz',
            'base_rhs': 'b-ub.npy',
            'bounds': 'bounds-original.npy',
            'column_scale': 'column-scale.npy',
            'metadata': 'lp-metadata.json',
            'time_seconds': float(time_seconds),
            'moment_order': int(moment_order),
            'n_actions': int(n_actions),
            'total_variables': int(total_vars),
            'lp_scaling_mode': str(lp_scaling),
            'goal_key': str(goal_key),
            'goal_coordinate_kind': str(goal_coordinate_kind),
            'lazy_tail': {
                'enabled': bool(
                    solve_mode == 'feasibility' and moment_order >= 1
                    and recipe_tail_capacity_specs
                ),
                'minimum_window_seconds': float(_tail_min_window),
                'separation_tolerance': 1e-6,
                'max_rounds_default': int(lazy_tail_max_rounds),
                'formulation': recipe_tail_capacity_meta['formulation'],
                'specs': recipe_tail_capacity_specs,
            },
        }
        (save_lp_dir / 'solver-package.json').write_text(
            json.dumps(_json_safe(solver_package), indent=2, sort_keys=True) + '\n',
            encoding='utf-8',
        )
        print(f'Saved pre-solve LP diagnostic snapshot to {save_lp_dir}', flush=True)

    if build_only:
        if save_lp_dir is None:
            raise ValueError('--build-only requires --save-lp-dir so the standalone solver has an LP package')
        return {
            'status': 'built',
            'solver_message': 'LP package built; HiGHS was not invoked',
            'goal_coordinate_kind': goal_coordinate_kind,
            'goal_key': goal_key,
            'num_actions': int(n_actions),
            'num_variables': int(total_vars),
            'num_inequalities': int(A_ub.shape[0]),
            'lp_package_dir': str(save_lp_dir),
            'standalone_solver': 'solve_saved_moment_lp_v1.py',
            'recipe_tail_capacity': recipe_tail_capacity_meta,
        }
    ...
    if direct_solver_backend not in {'auto', 'scipy', 'highspy'}:
        raise ValueError("--direct-solver-backend must be auto, scipy, or highspy")
    highspy_installed = importlib.util.find_spec('highspy') is not None
    if direct_feasibility:
        if direct_solver_backend == 'highspy':
            if not highspy_installed:
                raise RuntimeError('--direct-solver-backend highspy requested but highspy is not installed')
            resolved_direct_backend = 'highspy'
        elif direct_solver_backend == 'scipy':
            resolved_direct_backend = 'scipy'
        else:
            resolved_direct_backend = 'highspy' if highspy_installed else 'scipy'
    else:
        resolved_direct_backend = 'scipy'

    # Match the public --method flag in the native highspy path.  v33's default
    # is highs-ipm; plain highs also chooses IPM here because the direct
    # feasibility model is the numerically difficult path we are targeting.
    highspy_solver = 'simplex' if method == 'highs-ds' else 'ipm'
    ...
    print(
        ...
        f'direct backend={resolved_direct_backend}'
        + (
    ...
    def describe_constraint_row(row_i: int) -> dict[str, Any]:
        ...
        for group in constraint_groups:
            ...
            elif name.startswith('recipe_tail_capacity_lazy_round_'):
                lazy_rows = group.get('rows', [])
                if 0 <= local < len(lazy_rows):
                    out.update(dict(lazy_rows[local]))
    ...
    attempt = 0
    while attempt < ordering_attempts_effective:
        ...
        highspy_attempt_meta: dict[str, Any] | None = None
        same_instance_certificate: dict[str, Any] | None = None
        same_instance_cert_lambda: np.ndarray | None = None
        if direct_feasibility and resolved_direct_backend == 'highspy':
            highspy_attempt_meta, highspy_x, same_instance_cert_lambda = _highspy_direct_feasibility_attempt(
                ...
                certificate_time_limit=certificate_time_limit,
                solver_log=solver_log,
                request_certificate=infeasibility_certificate,
                solver=highspy_solver,
            )
            try_elapsed = float(highspy_attempt_meta.get('solve_elapsed_seconds', 0.0))
            hs_status = str(highspy_attempt_meta.get('status', 'nondecisive'))
            status_code = {
                'optimal': 0,
                'infeasible': 2,
                'nondecisive': 1,
            ...
            try_result = SimpleNamespace(
                status=status_code,
                x=highspy_x,
                fun=0.0 if status_code == 0 else None,
                message=highspy_attempt_meta.get('reason') or highspy_attempt_meta.get('model_status') or hs_status,
            )
            same_instance_certificate = highspy_attempt_meta.get('infeasibility_certificate')
        else:
        ...
        rec: dict[str, Any] = {
            ...
            'solver_backend': ('highspy' if highspy_attempt_meta is not None else 'scipy'),
        }
        if highspy_attempt_meta is not None:
            rec['highspy_attempt'] = highspy_attempt_meta
        ...
        if try_result.status == 0:
            if phase1_enabled:
                ...
                rec['solution_validation'] = validation
                if validation['passed']:
                    new_tail_cuts = _append_violated_lazy_tail_cuts(y_candidate)
                    if new_tail_cuts:
                        rec['status'] = 'lazy_tail_cuts_added'
                        rec['lazy_tail_cuts_added'] = new_tail_cuts
                        attempt_records.append(rec)
                        print(
                            f'HiGHS attempt {attempt + 1} found a base-feasible point, but '
                            f'{len(new_tail_cuts)} exact most-violated recipe tail-capacity cut(s) '
                            f'were separated; re-solving the same ordering attempt with the tightened LP.',
                            flush=True,
                        )
                        # Re-run the same numbered attempt.  The row count changed, so the
                        # deterministic permutation generated from this attempt seed changes only
                        # as required to include the newly added rows.
                        continue
                    accepted_result = try_result
                    accepted_y = y_candidate
                    accepted_validation = validation
                    attempt_records.append(rec)
                    print(
                        f'HiGHS attempt {attempt + 1} returned a validated optimum satisfying '
                        f'the full continuously separated tail-capacity family after '
            ...
            if infeasibility_certificate:
                if same_instance_certificate is not None:
                    print(
                        f'HiGHS attempt {attempt + 1} reported infeasible after '
                        f'{_format_elapsed(try_elapsed)}; checking the explicit IPM Farkas certificate LP.',
                        flush=True,
                    )
                    cert_meta = same_instance_certificate
                    cert_lambda = same_instance_cert_lambda
                else:
                    print(
                        f'HiGHS attempt {attempt + 1} reported infeasible after '
                        f'{_format_elapsed(try_elapsed)}; solving the explicit IPM Farkas certificate LP.',
                        flush=True,
                    )
                    cert_meta, cert_lambda = _highspy_ipm_farkas_certificate(
                        A_scaled_permuted=A_core_try,
            ...
            rec['status'] = 'numerical_difficulty'
            diagnostic_reason = (
                highspy_attempt_meta.get('reason')
                if highspy_attempt_meta is not None else None
            )
            print(
                f'HiGHS attempt {attempt + 1} was non-decisive ({try_status}) after '
                f'{_format_elapsed(try_elapsed)}'
                + (f'; highspy detail: {diagnostic_reason}' if diagnostic_reason else '')
                + '; retrying with a new ordering.',
                flush=True,
            )
        attempt_records.append(rec)
        attempt += 1
    ...
    if precheck_infeasible:
        ...
        solver_message = (
            'Infeasibility certified by an explicit IPM Farkas-certificate LP whose ordinary-row '
            'multipliers were independently verified against the original unscaled LP and variable bounds; '
        ...
        # v33 never turns raw solver votes into a proof.  In particular, IPM has
        # previously produced false infeasible statuses on ill-conditioned models.
        # Only the exact bound precheck or an independently verified Farkas
        # certificate may set status=infeasible.
        result = last_result
        y = None
        status = 'numerical_difficulty'
        solver_status_code = 4
        solver_message = (
            f'{infeasible_count} solver ordering(s) reported infeasible '
            f'(requested confirmations={infeasible_confirmations}), but no independent '
            'Farkas certificate was verified; treating the result as inconclusive rather '
            'than claiming a lower bound.'
        )
    else:
        result = last_result
        y = None
    ...
    output: dict[str, Any] = {
        ...
        'ordering_search': {
            ...
            'direct_solver_backend_requested': direct_solver_backend,
            'direct_solver_backend_resolved': resolved_direct_backend,
            'note': (
                ...
                + (
                    'An explicit IPM Farkas-certificate candidate that is independently verified against the original LP stops immediately as a proof. '
        ...
        'infeasibility_certificate': {
            ...
            'note': (
                'In direct mode, v33 uses highspy as the primary backend when available, so an infeasible '
                'status uses native highspy; infeasible screening is followed by an explicit IPM Farkas LP. If SciPy is explicitly '
                'selected or highspy is unavailable, v33 falls back to a checked highspy re-solve. In '
                'either case it undoes explicit row scaling and verifies the aggregate contradiction '
                'against the original unscaled LP using extended-precision arithmetic. A verified '
                'certificate is sufficient by itself; raw infeasibility votes are diagnostic only.'
            ),
        },
            ...
            (
                {
                    ...
                    'note': (
                        'v33 direct mode: solve the original feasibility LP directly with zero objective. '
        ...
        'recipe_tail_capacity': {
            **recipe_tail_capacity_meta,
            ...
            'separation_tolerance': float(lazy_tail_separation_tolerance),
            'max_rounds': int(lazy_tail_max_rounds),
        },
        'energy_flow_relaxation': energy_flow_meta,
        ...
        'technology_relaxation': {
            ...
            'generic_recipe_technology_constraint_count': len(generic_recipe_constraints),
            'all_recipe_technology_constraint_count': len(recipe_constraints),
            ...
            'interpretation': (
                ...
                'optimistic relaxation and preserves lower-bound validity. Recipes with positive exported amount lower bounds use the better-conditioned L_r technology-moment cuts; only remaining gated recipes use the loose U_r availability hierarchy. Fixed required copies retain their stronger known-mass timing constraints.'
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Keep explicit burner/electric/heat energy-source flow coordinates. v33 "
            "defaults to making energy free because those joule-scale carrier rows are "
            "not currently part of the lower-bound claim and badly condition the LP."
        ),
    )
    parser.add_argument(
        "--lazy-tail-max-rounds",
        type=int,
        default=50,
        help=(
            "Maximum continuous recipe-tail cut-separation rounds before refusing to "
            "accept a feasible candidate (default 50)."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="highs-ipm",
        help=(
            "HiGHS LP algorithm. v33 defaults to highs-ipm for direct feasibility; "
            "an IPM infeasible status is never accepted as proof without the independent "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "tests whether --goal-amount is reachable; v33 defaults to a direct zero-objective "
    ...
    parser.add_argument(
        ...
        choices=["auto", "highspy", "scipy"],
        default="auto",
        help=(
            "Backend for direct feasibility attempts (default auto). In v33, auto uses "
            "highspy when installed; v33 uses IPM for direct feasibility and certifies any "
            "the same HiGHS instance/version; otherwise it falls back to SciPy/HiGHS. "
            "Use scipy to reproduce the old v30 screening behavior explicitly."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable the v33 highspy Farkas-certificate check after a direct HiGHS "
            "infeasible report with a separately solved and independently verified Farkas certificate. "
    ...
    parser.add_argument(
        ...
        default=600.0,
        help=(
            "Time limit in seconds for dual-ray acquisition/certification after an "
            "infeasible direct solve (default 600). With the highspy direct backend this "
            "applies to the explicit IPM Farkas-certificate LP. Set to 0 for no "
            "certificate time limit. "
    ...
    parser.add_argument(
        ...
        default="direct",
        help=(
            "Feasibility strategy. direct (default in v33) solves the original LP with zero "
    ...
    parser.add_argument(
        "--keep-generic-u-tech-gates",
        action="store_true",
        help=(
            "Restore the older loose U_r technology-availability hierarchy for gated "
            "recipes that lack exported amount lower bounds. v33 disables it by default "
            "because those rows dominated the remaining 1e8-scale row dynamic range; "
            "omitting them is an optimistic, proof-safe relaxation."
        ),
    )
    parser.add_argument(
        "--no-tech-timing",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Build and save the LP package, then exit before invoking HiGHS. "
            "Requires --save-lp-dir. Use solve_saved_moment_lp_v1.py on that directory "
            "to solve/certify the frozen numerical problem separately."
        ),
    )
    parser.add_argument(
        "--active-eps",
    ...
    if args.build_only and args.save_lp_dir is None:
        raise ValueError("--build-only requires --save-lp-dir")
    ...
    rocket_part_recipe_goal = (
        args.goal_type == 'recipe' and args.goal == 'rocket-part'
        and float(args.goal_amount) >= 72.0 - 1e-9
    )
    ...
    if not args.no_action_lower_bounds:
        # The current exporter models the vanilla rocket progression under the
        # user's fixed 72-rocket-part productivity assumption.  It is valid for
        # either the historical one-launch target or the cleaner v33 target of
        # 72 executions of recipe: rocket-part.
        if (goal_key == 'launch: ' and float(args.goal_amount) >= 1.0) or rocket_part_recipe_goal:
            ...
            if rocket_part_recipe_goal:
                # The exporter includes launch:1 because its original presentation
                # target was a launch.  For a 72-rocket-part action target, requiring
                # a launch would re-introduce the vanilla 100-parts-per-launch graph
                # relation that this assumption set intentionally avoids.
                action_amount_lower_bound_export = dict(action_amount_lower_bound_export)
                lb_actions_copy = dict(action_amount_lower_bound_export.get('actions', {}))
                lb_actions_copy.pop('launch: ', None)
                action_amount_lower_bound_export['actions'] = lb_actions_copy
                action_amount_lower_bound_export['applied_goal_override'] = {
                    'goal': 'recipe: rocket-part',
                    'amount': float(args.goal_amount),
                    'removed_exported_action_bound': 'launch: ',
                }
            ...
            print(
                ...
                "is defined for a vanilla launch or a 72+ 'recipe: rocket-part' goal.",
                flush=True,
            )
    ...
    tech_enabled = bool(tech_requested and (args.goal_type in {"item", "recipe"} or launch_rocket_goal))
    ...
    modeled_techs: set[str] = set()
    if tech_enabled:
        if launch_rocket_goal or rocket_part_recipe_goal:
            ...
            if goal_unlock_tech not in tech_cone_costs:
                ...
                raise RuntimeError(
                    "Vanilla launch/rocket-part technology was hard-coded as 'rocket-silo', but no "
    ...
    if tech_enabled and modeled_techs:
        ...
        if launch_rocket_goal:
            ...
            if args.goal_type == 'recipe':
                goal_recipe_requirement = {
                    'recipe': args.goal,
                    'recipe_key': f'recipe: {args.goal}',
                    'yield_per_run': 1.0,
                    'required_runs': float(args.goal_amount),
                    'proof_condition': 'the requested goal is the recipe action itself',
                }
            else:
    ...
    result = solve_moment_growth_lp(
        ...
        direct_solver_backend=args.direct_solver_backend,
        relax_energy_flow=not args.keep_energy_flow,
        lazy_tail_max_rounds=args.lazy_tail_max_rounds,
        keep_generic_u_tech_gates=args.keep_generic_u_tech_gates,
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v33",
            ...
            "energy_flow_relaxed_free": bool(not args.keep_energy_flow),
            "lazy_tail_max_rounds": int(args.lazy_tail_max_rounds),
            "generic_u_tech_gates_enabled": bool(args.keep_generic_u_tech_gates),
    ...
    if args.solve_mode == "feasibility":
        if result["status"] == "infeasible":
            # In v33 status=infeasible is reserved for the exact variable-bound
            # contradiction precheck or an independently verified Farkas
            # certificate. A raw IPM infeasibility status is never promoted here.
            result["lower_bound_test"] = {
                ...
                "interpretation": (
                    ...
                    "therefore the real game requires more time. The infeasibility "
                    "claim was independently certified rather than inferred from the IPM status alone."
                ),
            }
