...
def _semantic_q_column_scales(
    ...
    action whose per-unit physical effect is s_r, choose D_r ~= median(s)/s_r so
    that one solver unit has an order-median physical effect.  Apply the SAME D_r
    ...
    mask = np.isfinite(raw) & (raw > 0.0)
    action_scale[mask] = reference / raw[mask]
    ...
    action_scale = np.clip(action_scale, 1e-8, 1e8)
    ...
    meta = {
        **effect_meta,
        'semantic_scaling_formula': 'D_r = median_positive_effect / physical_effect_r; original q = D_r * solver q',
...
def solve_moment_growth_lp(
    ...
    v13 eliminates big-M, uses action-aware q-variable scaling plus row equilibration, and in feasibility mode minimizes a positively scaled normalized action-flow objective instead of using the identically-zero objective. It also randomizes the final solver row/column ordering and retries independent permutations after numerical failures or before accepting infeasibility. This leaves the feasible set unchanged but strongly discourages gratuitous huge circulation flows. The goal recipe is split into a required copy
    ...
    if recipe_constraints:
        raise RuntimeError(
            'Internal v13 error: generic recipe_constraints should be empty; '
    ...
    if solve_mode == 'maximize':
        ...
        # The old feasibility objective was identically zero.  That gives HiGHS
        # no reason to avoid enormous null-space circulations (some previous
        # solutions had q0 values around 1e14), which is disastrous numerically.
        # A strictly positive objective changes only which feasible point is
        # selected; it cannot make an infeasible LP feasible or vice versa.
        flow_weights, flow_weight_meta = _normalized_action_flow_weights(
            S=S, C=C, time_seconds=time_seconds
        )
        ...
        max_w = float(np.max(flow_weights)) if flow_weights.size else 1.0
        objective_scale_factor = float(feasibility_objective_max) / max_w
        c[:n_actions] = flow_weights * objective_scale_factor
        flow_weight_meta = dict(flow_weight_meta)
        flow_weight_meta.update({
            'solver_objective_target_max': float(feasibility_objective_max),
            'solver_objective_global_scale_factor': objective_scale_factor,
            'solver_objective_min': float(np.min(c[:n_actions])) if n_actions else None,
            'solver_objective_max': float(np.max(c[:n_actions])) if n_actions else None,
            'note': 'The entire regularizer objective is multiplied by one positive constant; minimizers and feasibility are unchanged.',
        })
    ...
    # Variable-unit scaling multiplies objective coefficients by D.  Renormalize the
    # ENTIRE feasibility objective one final time so HiGHS still sees the requested
    # small cost scale.  This is one positive global factor and changes neither the
    # feasible set nor the minimizing feasible point.
    if solve_mode == 'feasibility' and c_solve.size:
        max_abs_solver_c = float(np.max(np.abs(c_solve)))
        if max_abs_solver_c > 0.0 and math.isfinite(max_abs_solver_c):
            post_column_objective_scale = float(feasibility_objective_max) / max_abs_solver_c
            c_solve *= post_column_objective_scale
            if flow_weight_meta is not None:
                flow_weight_meta = dict(flow_weight_meta)
                flow_weight_meta['post_column_global_scale_factor'] = post_column_objective_scale
                nz = np.abs(c_solve[np.nonzero(c_solve)])
                flow_weight_meta['solver_objective_min_after_column_scaling'] = (
                    float(np.min(nz)) if nz.size else 0.0
                )
                flow_weight_meta['solver_objective_max_after_column_scaling'] = (
                    float(np.max(nz)) if nz.size else 0.0
                )
    ...
    if semantic_scale_meta is not None:
        ...
        print(
            "Action-aware q scaling: physical effect "
    ...
    if flow_weight_meta is not None:
        print(
            "Feasibility regularizer=min normalized q0 action flow: "
            f"weights [{flow_weight_meta['weight_min']:.3g}, "
            f"{flow_weight_meta['weight_max']:.3g}], median="
            f"{flow_weight_meta['weight_median']:.3g}; solver objective range "
            f"[{flow_weight_meta.get('solver_objective_min_after_column_scaling', flow_weight_meta['solver_objective_min']):.3g}, "
            f"{flow_weight_meta.get('solver_objective_max_after_column_scaling', flow_weight_meta['solver_objective_max']):.3g}]",
            flush=True,
        )
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v13-pre-solve',
    ...
    print(
        ...
        f'inequalities={A_solve.shape[0]:,}, A_ub nnz={A_solve.nnz:,}; '
    ...
    def validate_original_solution(y_candidate: np.ndarray) -> dict[str, Any]:
        ...
        return {
            ...
            'interpretation': 'Returned optimum rechecked against the original unscaled LP; normalized residual uses the same positive row scaling only as a dimensionless error measure.',
        }
    ...
    for attempt in range(ordering_attempts):
        ...
        A_try = A_solve[row_perm, :][:, col_perm].tocsr()
        b_try = b_solve[row_perm]
        c_try = c_solve[col_perm]
        bounds_try = [bounds_solve[int(j)] for j in col_perm]
        ...
        if try_result.status == 0:
            ...
            if validation['passed']:
                ...
                print(
                    f'HiGHS attempt {attempt + 1} returned a validated feasible optimum after '
    ...
    if accepted_result is not None and accepted_y is not None:
        ...
        solver_message = (
            f'No validated feasible solution found, but only {infeasible_count} ordering(s) reported '
            f'infeasible (required {infeasible_confirmations}); treating result as numerical difficulty.'
        )

    print(
        f'HiGHS ordering search finished after {_format_elapsed(total_solve_elapsed)}: '
        f'status={status}; infeasible confirmations={infeasible_count}/{ordering_attempts}',
        flush=True,
    )
    ...
    output: dict[str, Any] = {
        ...
        'ordering_search': {
            ...
            'note': 'Permuting rows/columns does not change the LP; it changes only the simplex/presolve path. Any validated feasible solution overrides infeasibility reports from other orderings.',
        },
        'feasibility_objective': (
            {
                'type': 'minimum_normalized_action_flow',
                'changes_feasible_set': False,
                'weighting': flow_weight_meta,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "tests whether --goal-amount is reachable while minimizing a positive normalized action-flow regularizer to suppress gratuitous circulation; it is often much faster "
    ...
    parser.add_argument(
        ...
        default=1e-3,
        help=(
            "Scale the entire minimum-flow feasibility objective so its largest "
            "raw coefficient is this value (default 1e-3). This positive global "
            "rescaling leaves the minimizers and feasible set unchanged but avoids "
            "excessive simplex dual values."
        ),
    )
    parser.add_argument(
        "--ordering-seed",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v13 and ignored. The required-goal-recipe split removes "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v13; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v13's no-big-M technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_recipe_requirement is None or goal_unlock_tech is None:
            raise RuntimeError('Internal v13 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v13",
