    ...
    with path.open('rb') as fh:
        for raw in fh:
            ...
            if pending_tech is not None:
                parsed = _parse_serpent_numeric_map(line)

                # "{}" / empty serialized table is also a legitimate technology cost.
                if parsed or "return _" in line or line.lstrip().startswith("{"):
                    costs[pending_tech] = parsed
                    pending_tech = None
    return costs
...
def augment_matrices_with_technology_research(
    ...
    """Append one fractional research action per modeled technology."""
...
def solve_moment_growth_lp(
    ...
    tech_big_m: float | None = None,
) -> dict[str, Any]:
    """Solve the K-moment optimistic continuous-time production relaxation.

    If ``tech_info`` is supplied, one fractional research action exists for each
    modeled technology.  Real schedules correspond to activation y_T in {0,1};
    this LP relaxes y_T to [0,1].  Completion-moment variables h[T,k] represent
    y_T * ((delta-t_unlock)/delta)^k in real schedules.

    A recipe uniquely gated by T obeys the valid big-M moment inequalities

        q_r,0 <= M y_T
        q_r,k <= M h[T,k].

    M is either user-supplied or obtained by one optimistic pre-solve maximizing
    the total amount of all gated recipes.  Because that pre-solve has fewer
    constraints, its optimum is a valid common upper bound for every gated
    recipe in the final LP.
    """
    if moment_order < 1:
    ...
    for tech in tech_names:
        ...
        bounds[ai] = (0.0, 1.0)  # y_T = q_0 of fractional research action
    for tech in tech_names:
        for k in range(1, moment_order + 1):
            bounds[h_index(tech, k)] = (0.0, 1.0)

    # Determine a rigorous common M for recipe activation/timing.  Do this from
    # the production-only relaxation BEFORE adding technology constraints.
    tech_M: float | None = None
    tech_M_meta: dict[str, Any] = {'recipe_constraint_count': len(recipe_constraints)}
    if recipe_constraints:
        if tech_big_m is not None:
            if not math.isfinite(tech_big_m) or tech_big_m <= 0.0:
                raise ValueError('--tech-big-m must be finite and > 0')
            tech_M = float(tech_big_m)
            tech_M_meta.update({
                'source': 'user-supplied', 'value': tech_M,
                'rigor_warning': (
                    'A user-supplied M preserves the lower-bound proof only if it is '
                    'at least the true production-only upper bound for every gated recipe.'
                ),
            })
        else:
            base_A = vstack(constraint_mats, format='csr')[:, :q_total_vars]
            base_b = np.concatenate(constraint_rhs)
            pre_c = np.zeros(q_total_vars, dtype=float)
            gated_indices = sorted({int(r) for r, _recipe, _tech in recipe_constraints})
            for r in gated_indices:
                pre_c[r] = -1.0
            pre_bounds: list[tuple[float, float | None]] = [(0.0, None)] * q_total_vars
            # Technology pseudo-actions are not part of the production-only bound.
            for tech in tech_names:
                ai = tech_action_index[tech]
                for k in range(blocks):
                    pre_bounds[k * n_actions + ai] = (0.0, 0.0)
            pre_options: dict[str, Any] = {'disp': bool(solver_log)}
            if solver_time_limit is not None:
                pre_options['time_limit'] = float(solver_time_limit)
            print(
                f'Computing automatic technology big-M bound for '
                f'{len(gated_indices):,} gated recipe action(s)...', flush=True,
            )
            pre_result, pre_elapsed = _linprog_with_heartbeat(
                c=pre_c, A_ub=base_A, b_ub=base_b, method=method,
                solver_options=pre_options, heartbeat_seconds=heartbeat_seconds,
                label='Tech M pre-solve', bounds=pre_bounds,
            )
            if pre_result.status != 0:
                raise RuntimeError(
                    'Could not obtain a finite rigorous automatic technology big-M '
                    f'bound: {pre_result.message}. You may pass --tech-big-m only if '
                    'you have an independently valid upper bound.'
                )
            raw_M = max(0.0, -float(pre_result.fun))
            tech_M = max(1.0, raw_M * 1.001 + 1e-7)
            tech_M_meta.update({
                'source': 'production-only LP upper bound on sum of gated recipe amounts',
                'raw_sum_upper_bound': raw_M,
                'value': tech_M,
                'solve_elapsed_seconds': float(pre_elapsed),
                'safety_inflation_factor': 1.001,
            })
            print(f'Technology big-M = {tech_M:.9g}', flush=True)
    ...
    if tech_names:
        ...
        if recipe_constraints and tech_M is not None:
            rows = []; rhs = []
            for r, _recipe, tech in recipe_constraints:
                ai = tech_action_index[tech]
                rows.append({r: 1.0, ai: -tech_M}); rhs.append(0.0)
                for k in range(1, moment_order + 1):
                    rows.append({k * n_actions + r: 1.0, h_index(tech, k): -tech_M}); rhs.append(0.0)
            add_sparse_rows(
                'recipe_after_unique_unlock_technology', rows, rhs,
                big_m=float(tech_M), gated_recipe_count=len(recipe_constraints),
            )
    ...
    print(
        ...
        f'inequalities={A_ub.shape[0]:,}, A_ub nnz={A_ub.nnz:,}', flush=True,
    )
    result, solve_elapsed = _linprog_with_heartbeat(
        c=c, A_ub=A_ub, b_ub=b_ub, method=method,
        solver_options=solver_options, heartbeat_seconds=heartbeat_seconds,
        label='HiGHS', bounds=bounds,
    )
    ...
    output: dict[str, Any] = {
        ...
        'technology_relaxation': {
            ...
            'gated_recipe_count': len(recipe_constraints),
            'big_m': tech_M_meta if recipe_constraints else None,
            'interpretation': (
                'Technology activation is fractionally relaxed. Real schedules (0/1 '
                'research decisions and exact unlock times) are contained in this LP, '
                'so infeasibility remains a valid lower-bound certificate.'
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable TECHNOLOGY cone-cost parsing and unique recipe-unlock timing "
            "constraints; useful for comparison with v6 behavior."
        ),
    )
    parser.add_argument(
        "--tech-big-m",
        ...
        default=None,
        help=(
            "Optional common upper bound for every uniquely tech-gated recipe "
            "action total. By default v7 computes a rigorous bound with one "
            "production-only LP pre-solve. Supplying this skips that pre-solve, "
            "but the lower-bound proof is valid only if your bound is truly large enough."
        ),
    )
    parser.add_argument(
        "--output",
    ...
    if args.no_goal_prune:
        ...
        if tech_enabled:
            relevant_recipes = {
                node.name for node in stage_nodes.values()
                if node.node_type == "recipe" and node.name is not None
            }
            modeled_techs = {
                recipe_to_tech_all[r] for r in relevant_recipes
            ...
            for tech in list(modeled_techs):
                modeled_techs.update(tech_ancestors.get(tech, set()))
    elif tech_enabled:
        stage_nodes, stage_edges, modeled_techs, goal_prune_meta = (
            technology_quantitative_prune_closure(
                stage_nodes, stage_edges, goal_key, recipe_to_tech_all,
                tech_direct_costs, tech_ancestors,
            )
        )
    else:
    ...
    if tech_enabled:
        print(
            f"Technology closure: {len(modeled_techs):,} tech(s), "
            f"{len(recipe_to_tech_all):,} uniquely tech-gated recipe(s) detected "
            f"({len(ambiguous_recipe_unlocks):,} ambiguous/alternative unlocks left free)",
            flush=True,
        )
    ...
    result = solve_moment_growth_lp(
        ...
        tech_info=tech_info,
        tech_big_m=args.tech_big_m,
    )
    phase_timings["build_and_solve_lp"] = time.perf_counter() - phase_start

    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v7",
