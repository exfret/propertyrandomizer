...
In addition to Hausdorff consistency of each action-moment sequence q_k, v23
...
v23 also derives a deliberately optimistic but finite real-time execution-rate
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v24-pre-solve',
...
def main() -> None:
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v24",
