...
For edges carrying ``used_payback``, the dumped amount is interpreted as having
already been divided by that payback.  Before graph normalization the coefficient
is rebased to the requested candidate time delta as

    effective_amount = stored_amount * used_payback / delta.

Thus a dump made with any payback can be reused for different ``--time-seconds``
values. Edges without ``used_payback`` retain their stored amount unchanged.

Let z >= 0 be total action usage over [0, delta]. The relaxed final finite
inventory is
...
omitted.  The character is explicitly excluded from that infinite set, and
its explicit-zero incoming edge is NOT turned into a repeatable free producer;
it only establishes the separate initial inventory of one character unit.
...
--time-seconds actively rebases every edge carrying ``used_payback``.  For
example, a stored amount ``recipe.energy_required / payback_time`` with
``used_payback = payback_time`` becomes ``recipe.energy_required / delta``.
"""
...
def rebase_used_payback_edges(
    edges: Sequence[GraphEdge],
    time_seconds: float,
) -> tuple[list[GraphEdge], dict[str, Any]]:
    """Rebase payback-amortized edge amounts to the requested candidate time.

    An edge dumped as ``amount = raw_amount / used_payback`` becomes
    ``raw_amount / time_seconds``.  Edges without ``used_payback`` are unchanged.
    Rebasing happens before graph normalization so every synthetic transformation
    sees the already-correct quantitative coefficient.
    """
    ...
    paybacks: list[float] = []
    scales: list[float] = []
    before_amounts: list[float] = []
    after_amounts: list[float] = []
    ...
    for edge in edges:
        ...
        scale = used / time_seconds
        rebased_amount = edge.amount * scale
        if not math.isfinite(rebased_amount):
            raise RuntimeError(
                f"Rebasing edge {edge.key!r} produced non-finite amount"
            )
        ...
        before_amounts.append(float(edge.amount))
        after_amounts.append(float(rebased_amount))
        if len(sample) < 12:
            sample.append(
                {
                    "edge_key": edge.key,
                    ...
                    "scale_used_payback_over_time": float(scale),
                    "effective_amount": float(rebased_amount),
                }
            )

    if paybacks:
        meta: dict[str, Any] = {
            "rebased_edge_count": len(paybacks),
            "formula": "effective_amount = stored_amount * used_payback / time_seconds",
            ...
            "stored_amount_abs_range": {
                "min": min(abs(x) for x in before_amounts),
                "max": max(abs(x) for x in before_amounts),
            },
            "effective_amount_abs_range": {
                "min": min(abs(x) for x in after_amounts),
                "max": max(abs(x) for x in after_amounts),
            },
            "sample": sample,
        }
    else:
        meta = {
            "rebased_edge_count": 0,
            "formula": "effective_amount = stored_amount * used_payback / time_seconds",
            "time_seconds": float(time_seconds),
            "note": "No stage edge carried used_payback; time_seconds did not alter S.",
            "sample": [],
        }
    return out, meta
...
def solve_relaxed_growth_lp(
    ...
    if result.status != 0:
        if result.status == 3:
            # Exhibit an unbounded direction d when possible.  Unboundedness of
            # the original LP means there is a nonnegative action direction with
            # nonnegative net change in every finite material and positive goal
            # production.  Normalize it to >=1 goal and minimize L1 action usage
            # so the returned certificate is reasonably small/readable.
            ray_A = vstack([
                (-N[finite_indices, :]).tocsr(),
                csr_matrix((-goal_coeff).reshape(1, -1)),
            ], format="csr")
            ray_b = np.concatenate([
                np.zeros(finite_indices.size, dtype=float),
                np.array([-1.0], dtype=float),
            ])
            ray_result = linprog(
                c=np.ones(n_actions, dtype=float),
                A_ub=ray_A,
                b_ub=ray_b,
                bounds=(0.0, None),
                method=method,
            ...
            if ray_result.status == 0:
                d = np.asarray(ray_result.x, dtype=float)
                change = np.asarray(N @ d, dtype=float).reshape(-1)
                ...
                ray_actions.sort(
                    key=lambda item: item["amount_per_unit_goal_ray"],
                    reverse=True,
                )

                finite_changes = []
                infinite_changes = []
                for j, value in enumerate(change):
                    if abs(float(value)) <= active_eps:
                        continue
                    entry = {
                        "material_key": col_keys[j],
                        "net_change_per_unit_goal_ray": float(value),
                    }
                    if j in infinite_indices:
                        infinite_changes.append(entry)
                    else:
                        finite_changes.append(entry)
                finite_changes.sort(
                    key=lambda item: abs(item["net_change_per_unit_goal_ray"]),
                    reverse=True,
                )
                infinite_changes.sort(
                    key=lambda item: abs(item["net_change_per_unit_goal_ray"]),
                    reverse=True,
                )
                ray_info.update({
                    "goal_net_production": float(goal_coeff @ d),
                    "action_l1": float(np.sum(d)),
                    "minimum_finite_net_change": (
                        float(np.min(change[finite_indices]))
                        if finite_indices.size else math.inf
                    ),
                    "character_net_change": float(change[character_index]),
                    "active_actions": ray_actions,
                    "finite_material_net_changes": finite_changes,
                    "infinite_material_net_changes": infinite_changes,
                    "interpretation": (
                        "This nonnegative action combination can be scaled arbitrarily: "
                        "it produces positive goal while decreasing no finite material. "
                        "Negative changes are permitted only on infinite/autoplaced materials."
                    ),
                })
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Candidate lower-bound time delta in seconds. Every stage edge carrying "
            "used_payback is rebased from its stored payback to this delta before "
            "the LP matrix is built."
        ),
    )
    parser.add_argument(
        "--goal-amount",
    ...
    stage_edges, payback_meta = rebase_used_payback_edges(
        stage_edges, args.time_seconds
    )
    print(
        f"Rebased {payback_meta['rebased_edge_count']} used_payback edge(s) "
        f"to delta={args.time_seconds:g}s",
        flush=True,
    )
    transformed_nodes, transformed_edges, transform_meta = make_bipartite(
        stage_nodes, stage_edges, zero_free_excluded_or_keys={character_key}
    )
    ...
    result.update(
        {
            "format": "propertyrandomizer-relaxed-growth-lp-v3",
            ...
            "time_seconds_role": (
                "active: edges carrying used_payback are rebased by "
                "used_payback / time_seconds before normalization"
            ),
            "payback_rebasing": payback_meta,
