...
def make_bipartite(
    ...
    *,
    full_quantitative_input_nodes: set[str] | None = None,
) -> tuple[dict[str, GraphNode], list[GraphEdge], dict[str, int]]:
    """
    Reify explicitly cost-bearing OR nodes, then subdivide remaining nonzero
    same-op quantitative edges.

    An edge with no explicit ``amount`` is not part of the cost LP. Explicit
    ``amount = 0`` edges are likewise quantitatively inert.

    Cost-bearing OR semantics are exactly:

      non-source OR M(cost):

          M_pre(OR) --1--> M_cost(AND, cost) --1--> M(OR)

        Quantitative producers that originally ended at M are redirected to
        M_pre. Quantitative consumers still start from M.

      quantitative-source OR M(cost):

          M_cost(AND, cost) --1--> M(OR)

        The first OR is simply omitted. There is NO extra zero-cost source row.

    "Source" here means that the OR has no nonzero amount-bearing incoming edge
    in the FULL graph for this stage, before reachability filtering. Missing-
    amount reachability edges do not count as quantitative producers.
    """
    ...
    if full_quantitative_input_nodes is None:
        quantitative_input_nodes = {
            edge.stop
            for edge in edges
            if edge.amount_present and edge.amount != 0.0
        }
    else:
        quantitative_input_nodes = full_quantitative_input_nodes

    source_cost_or_nodes = {
        key for key in cost_or_nodes if key not in quantitative_input_nodes
    }
    nonsource_cost_or_nodes = cost_or_nodes - source_cost_or_nodes

    # Replace the node-level cost by an AND row. The original OR key remains the
    # downstream state so all consumers and target lookups continue to use it.
    for key in sorted(cost_or_nodes):
        ...
        if key in source_cost_or_nodes:
            # Special source rule: drop the first OR entirely.
            intermediate_edges.append(
                GraphEdge(
                    key=f"node-cost:{key}:source",
                    start=row,
                    stop=key,
                    amount=1.0,
                    amount_present=True,
    ...
    for edge in edges:
        ...
        # Only non-source cost ORs have the prerequisite-side OR. A true source
        # cost OR cannot have a quantitative incoming edge by definition.
        stop = (
            _synthetic_key("or_cost_pre", edge.stop)
            if edge.stop in nonsource_cost_or_nodes
            else edge.stop
        )
        intermediate_edges.append(
            GraphEdge(
    ...
    return out_nodes, strict_edges, {
        ...
        "source_cost_or_rows": len(source_cost_or_nodes),
        "nonsource_cost_or_bridges": len(nonsource_cost_or_nodes),
        "source_cost_or_detection": (
            "full-stage-graph"
            if full_quantitative_input_nodes is not None
            else "filtered-stage-subgraph"
        ),
    ...
    for stage_no in selected:
        ...
        try:
            ...
            transformed_nodes, transformed_edges, transform_meta = make_bipartite(
                stage_nodes,
                stage_edges,
                full_quantitative_input_nodes=full_quantitative_input_nodes,
            )
