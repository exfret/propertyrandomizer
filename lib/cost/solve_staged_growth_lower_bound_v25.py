...
In addition to Hausdorff consistency of each action-moment sequence q_k, v25
...
v25 also derives a deliberately optimistic but finite real-time execution-rate
...
v25 additionally treats synthetic conserved AND outputs, recipe-category,
...
def solve_moment_growth_lp(
    ...
    v23 keeps the fixed-copy technology timing and global-slack
    ...
    # In feasibility mode, explicitly construct a Phase-I LP.  The default v23
    # formulation uses ONE global normalized violation slack s:
    #
    ...
    if phase1_mode not in {'global', 'per-row'}:
        raise ValueError("phase1_mode must be 'global' or 'per-row'")
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v25-pre-solve',
                ...
                'feasibility_strategy': (f'explicit_phase1_{phase1_mode}' if phase1_enabled else None),
                'phase1_mode': phase1_mode if phase1_enabled else None,
    ...
    output: dict[str, Any] = {
        ...
        'ordering_search': {
            ...
            'note': (
                ...
                'In feasibility mode, each attempted ordering solves an explicit Phase-I LP. '
                'Any validated zero-violation feasible solution overrides positive Phase-I reports from other orderings. '
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="global",
        help=(
            "Phase-I formulation in feasibility mode. global (default) adds one slack shared "
            "by all normalized inequalities and minimizes the maximum violation; per-row keeps "
            "the older one-slack-per-row L1 diagnostic formulation, which can be much slower."
        ),
    )
    parser.add_argument(
        "--no-goal-prune",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v23 and ignored. Fixed-copy timing no longer needs "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v23; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v23's technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_unlock_tech is None:
            raise RuntimeError('Internal v23 error: missing mandatory goal technology')
        if launch_rocket_goal:
            if goal_action_requirement is None:
                raise RuntimeError('Internal v23 error: missing required goal action metadata')
            ...
            if goal_recipe_requirement is None:
                raise RuntimeError('Internal v23 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v25",
