...
def solve_moment_growth_lp(
    ...
    # Determine a rigorous M only for the mandatory named goal recipe timing.
    # Optional technology-gated recipes are not constrained at all in v8.
    ...
    if recipe_constraints:
        if tech_big_m is not None:
            ...
            print(
                f'Computing automatic timing bound for the mandatory goal recipe '
                f'({len(gated_indices):,} gated action(s))...', flush=True,
            )
            ...
            print(f'Mandatory-goal recipe timing M = {tech_M:.9g}', flush=True)
    ...
    output: dict[str, Any] = {
        ...
        'technology_relaxation': {
            ...
            'interpretation': (
                ...
                'optimistic relaxation and preserves lower-bound validity.'
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional upper bound for the named goal recipe action total, used only "
            "to enforce recipe-after-mandatory-tech timing. By default v8 computes "
            "a rigorous bound with one production-only LP pre-solve. Optional-tech "
            "recipes are not gated and do not participate in this bound."
        ),
    )
    parser.add_argument(
        "--output",
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v8",
