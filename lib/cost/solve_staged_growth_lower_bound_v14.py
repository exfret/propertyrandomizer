...
def _semantic_q_column_scales(
    ...
    meta = {
        **effect_meta,
        'semantic_scaling_formula': 'D_r = sqrt(median_positive_effect / physical_effect_r), clipped to [1e-3,1e3]; original q = D_r * solver q',
...
def solve_moment_growth_lp(
    ...
    if recipe_constraints:
        raise RuntimeError(
            'Internal v14 error: generic recipe_constraints should be empty; '
    ...
    if semantic_scale_meta is not None:
        ...
        print(
            "Conservative action-aware q scaling (sqrt): physical effect "
    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v14-pre-solve',
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v14 and ignored. The required-goal-recipe split removes "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v14; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v14's no-big-M technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_recipe_requirement is None or goal_unlock_tech is None:
            raise RuntimeError('Internal v14 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v14",
