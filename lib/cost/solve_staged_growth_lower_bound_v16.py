    ...
    if save_lp_dir is not None:
        ...
        (save_lp_dir / 'lp-metadata.json').write_text(
            json.dumps(_json_safe({
                'format': 'propertyrandomizer-moment-growth-lp-v16-pre-solve',
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v16 and ignored. The required-goal-recipe split removes "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v16; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v16's no-big-M technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_recipe_requirement is None or goal_unlock_tech is None:
            raise RuntimeError('Internal v15 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v16",
