...
LP semantics are taken directly from logic.graph:
  ...
  * every other original AND node with at least one quantitative outgoing edge
    is given one conserved synthetic OR output commodity; its outgoing uses are
    routed through that commodity so one execution cannot be spent several ways
    at once. This also gives AND targets a net-output commodity to price.
