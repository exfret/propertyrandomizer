...
def solve_moment_growth_lp(
    ...
    v9 eliminates big-M entirely. The goal recipe is split into a required copy
    ...
    # No big-M pre-solve in v9.  The required goal-recipe copy has a known
    # zeroth moment R, so its timing moments can be scaled directly by R.
    if recipe_constraints:
        raise RuntimeError(
            'Internal v9 error: generic recipe_constraints should be empty; '
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v9 and ignored. The required-goal-recipe split removes "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v9; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v9's no-big-M technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_recipe_requirement is None or goal_unlock_tech is None:
            raise RuntimeError('Internal v9 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v9",
