...
def _normalized_action_flow_weights(
    ...
    q0 amounts for different graph actions can live on wildly different numerical
    scales.  A route whose physical coefficients are ~1e-7 may need ~1e7 units of
    q0 to carry the same material flow as an action with order-one coefficients.
    Plain sum(q0) would therefore strongly prefer some unit conventions over
    others.

    Use each action's largest ordinary material coefficient or normalized
    capacity coefficient C/delta as its characteristic effect size, divide by the
    median positive effect size, and clip only for numerical conditioning.  Thus
    q0 * weight is roughly "amount of normalized physical flow".  Every weight is
    strictly positive, so the objective removes gratuitous circulation without
    ...
    if material.size != capacity.size:
        raise RuntimeError('Action-weight dimensions are inconsistent')
    ...
    # This is only a tie-breaking/regularizing objective.  Clipping preserves
    # positivity (hence preserves the feasible set) while avoiding a new extreme
    # cost-coefficient range of its own.
    normalized = np.clip(normalized, 1e-3, 1e3)
    meta = {
        'definition': 'max(max_abs_material_coefficient, max_capacity_duration/delta) / median_positive_effect, clipped to [1e-3,1e3]',
        'reference_median_effect': reference,
        'raw_effect_min_positive': float(np.min(positive)) if positive.size else None,
        'raw_effect_max': float(np.max(positive)) if positive.size else None,
        ...
        'zero_effect_action_count': int(np.count_nonzero(~mask)),
    }
...
def solve_moment_growth_lp(
    ...
    if recipe_constraints:
        raise RuntimeError(
            'Internal v11 error: generic recipe_constraints should be empty; '
    ...
    if flow_weight_meta is not None:
        print(
            ...
            f"[{flow_weight_meta['solver_objective_min']:.3g}, "
            f"{flow_weight_meta['solver_objective_max']:.3g}]",
            flush=True,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Deprecated in v12 and ignored. The required-goal-recipe split removes "
    ...
    if args.tech_big_m is not None:
        print("WARNING: --tech-big-m is deprecated and ignored in v11; no big-M solve is used.", flush=True)
    ...
    if tech_enabled and modeled_techs:
        if args.solve_mode != "feasibility":
            raise RuntimeError(
                "v11's no-big-M technology timing is designed for --solve-mode feasibility. "
    ...
    if tech_enabled and modeled_techs:
        ...
        if goal_recipe_requirement is None or goal_unlock_tech is None:
            raise RuntimeError('Internal v11 error: missing required goal recipe metadata')
    ...
    result.update(
        {
            "format": "propertyrandomizer-moment-growth-lp-v12",
