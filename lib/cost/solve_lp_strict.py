...
By default the script targets every reachable item/fluid OR node plus every
original node whose `cost` field is explicitly present (including AND nodes such
as `entity-operate`).  Cost-bearing AND nodes are priced by forcing one execution
of that row while requiring the full production plan to cover all quantitative
inputs.
...
def cost_target_keys(all_nodes: dict[str, GraphNode]) -> list[str]:
    """
    Keep the old item/fluid material targets, and additionally price every
    original node that carries a direct nonzero cost.  In the current graph
    this includes entity and entity-operate nodes.
    """
    return sorted(
        key
...
def main() -> None:
    ...
    print(
        ...
        f"cost targets={len(original_targets)} (items/fluids + direct-cost nodes)",
        flush=True,
    )
