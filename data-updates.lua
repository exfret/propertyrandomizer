...
local techs_done = 1
for _, pebble2 in pairs(init_sort.sorted) do
    local node2 = graph.nodes[pebble2.node_key]
    if node2.type == "technology" then
        local tech = data.raw.technology[node2.name]
        local is_okay = {}
        ...
        local short_path = consistent_sort.path(graph, goal_inds, init_sort, false)
        for ind = 1, #init_sort.sorted do
            local pebble = init_sort.sorted[ind]
            local node_key = pebble.node_key
            local node = graph.nodes[node_key]
            if node.type == "technology" and ind < goal_inds[1] then--and short_path.in_path[ind] then
                is_okay[node.name] = true
            end
        end
        log(serpent.block(is_okay))
        local blacklist_graph = table.deepcopy(graph)
        for _, tech2 in pairs(data.raw.technology) do
            if not is_okay[tech2.name] then
                gutils.add_edge(blacklist_graph, gutils.key("false", ""), gutils.key("technology", tech2.name))
            end
        end
        local new_sort = consistent_sort.sort(blacklist_graph)
        local reachable_in_order = {}
        for _, pebble in pairs(new_sort.sorted) do
            local node_key = pebble.node_key
            local node = graph.nodes[node_key]
            local blacklist_item_fluids = {
                ["water"] = true,
                ["steam"] = true,
                ["depleted-uranium-fuel-cell"] = true,
            }
            if (node.type == "item" or node.type == "fluid") and not blacklist_item_fluids[node.name] then
        ...
        for i = math.max(1, math.floor(0 * #reachable_in_order)), #reachable_in_order do
            TECH_TO_REACHABLE_NAME[tech.name][reachable_in_order[i]] = false
        end
        TECH_TO_REACHABLE_NAME[tech.name][reachable_in_order[#reachable_in_order]] = false
